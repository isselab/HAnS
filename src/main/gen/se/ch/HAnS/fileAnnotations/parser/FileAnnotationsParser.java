...
package se.ch.HAnS.fileAnnotations.parser;
...
public class FileAnnotationsParser implements PsiParser, LightPsiParser {
  ...
  static boolean parse_root_(IElementType t, PsiBuilder b, int l) {
    return fileAnnotationsFile(b, l + 1);
  }
  ...
  static boolean fileAnnotationsFile(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "fileAnnotationsFile")) return false;
    while (true) {
      int c = current_position_(b);
      if (!fileAnnotationsFile_0(b, l + 1)) break;
      if (!empty_element_parsed_guard_(b, "fileAnnotationsFile", c)) break;
    }
    return true;
  }

  // fileAnnotation NEWLINE*
  private static boolean fileAnnotationsFile_0(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "fileAnnotationsFile_0")) return false;
    boolean r;
    Marker m = enter_section_(b);
    r = fileAnnotation(b, l + 1);
    r = r && fileAnnotationsFile_0_1(b, l + 1);
    exit_section_(b, m, null, r);
    return r;
  }

  // NEWLINE*
  private static boolean fileAnnotationsFile_0_1(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "fileAnnotationsFile_0_1")) return false;
    while (true) {
      int c = current_position_(b);
      if (!consumeToken(b, NEWLINE)) break;
      if (!empty_element_parsed_guard_(b, "fileAnnotationsFile_0_1", c)) break;
    }
    return true;
  }
